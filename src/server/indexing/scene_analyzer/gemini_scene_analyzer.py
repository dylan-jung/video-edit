...
def analyze_video_with_gemini(video_path: str, chunk_index: int, model: str = "gemini-2.0-flash") -> Dict[str, Any]:
    """
    ...
    try:
        if "```json" in second_response.text:
            analysis_result = json.loads(second_response.text.split("```json")[1].split("```")[0])
        else:
            analysis_result = json.loads(second_response.text)
